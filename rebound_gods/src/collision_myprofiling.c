...
#include <time.h>
double time_nearest_neighbor = 0.0;
...
void reb_collision_search(struct reb_simulation* const r){
    ...
    double time0 = tim.tv_sec + (tim.tv_usec / 1000000.0);
    double time_cumu0 = 0.0;
    double time_cumu1 = 0.0;
    double time_cumu2 = 0.0;
        ...
        case REB_COLLISION_TREE:
            {
...
#endif // MPI
                gettimeofday(&tim, NULL);
                double time00 = tim.tv_sec + (tim.tv_usec / 1000000.0);
                ...
                gettimeofday(&tim, NULL);
                double time10 = tim.tv_sec + (tim.tv_usec / 1000000.0);
                time_cumu0 += (time10 - time00);
...
#pragma omp parallel for schedule(guided)
                for (int i=0;i<N;i++){
                    ...
                    double local_time_cumu1 = 0.0;
                    double local_time_cumu2 = 0.0;
                    ...
                    for (int gbx=-N_ghost_xcol; gbx<=N_ghost_xcol; gbx++){
                        for (int gby=-N_ghost_ycol; gby<=N_ghost_ycol; gby++){
                            for (int gbz=-N_ghost_zcol; gbz<=N_ghost_zcol; gbz++){
                                ...
                                gettimeofday(&tim, NULL);
                                double time10 = tim.tv_sec + (tim.tv_usec / 1000000.0);
                                local_time_cumu1 += (time10 - time00);
                                gettimeofday(&tim, NULL); // overlap
                                double time20 = tim.tv_sec + (tim.tv_usec / 1000000.0);
                                ...
                                gettimeofday(&tim, NULL); // 
                                double time30 = tim.tv_sec + (tim.tv_usec / 1000000.0);
                                local_time_cumu2 += (time30 - time20);
                            }
                        }
                    }
...
#pragma omp atomic
                    time_cumu1 += local_time_cumu1;
#pragma omp atomic
                    time_cumu2 += local_time_cumu2;
                }
            }
            break;
...
#pragma omp parallel for schedule(guided)
    ...
    gettimeofday(&tim, NULL);
    double timef = tim.tv_sec + (tim.tv_usec / 1000000.0);
    printf("coll search rt: %.3f s {multiple threads: 1- prep: %.4f s | 2- get gb: %.4f s | 3- check overlap: %.4f s} | ", 
        (timef - time0), time_cumu0, time_cumu1, time_cumu2);
    ...
    gettimeofday(&tim, NULL);
    double timef2 = tim.tv_sec + (tim.tv_usec / 1000000.0);
    printf("coll resolve rt: %.5f s | ", (timef2 - timef));
    printf("routine rt: %.3f s\n", (timef2 - time0));
}
