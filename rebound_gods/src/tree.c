    ...
    if (reb_tree_particle_is_inside_cell(r, node) == 0) {
        ...
        if (r->N){ // Check if there remains any particle in the simulation 
            ...
            if ((r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) && r->ri_lfada.par_jerk != NULL) {
                ...
                r->ri_lfada.par_jerk_reinsert = r->ri_lfada.par_jerk[oldpos];
                //r->ri_lfada.hash_reinsert = r->ri_lfada.hash_particles[oldpos]; // for debug only
                r->ri_lfada.par_jerk[oldpos] = r->ri_lfada.par_jerk[r->N];
                //r->ri_lfada.hash_particles[oldpos] = r->ri_lfada.hash_particles[r->N]; // for debug only
            }
            if ((r->ri_lfada.mode) && r->ri_lfada.par_soft_acc != NULL) {
                r->ri_lfada.par_soft_reinsert = r->ri_lfada.par_soft_acc[oldpos];
                r->ri_lfada.par_soft_acc[oldpos] = r->ri_lfada.par_soft_acc[r->N];
            }
    ...
    if (node->pt < 0) {
        ...
        for (int o=0; o<8; o++) {
            ...
            if (d!=NULL){
                ...
                // Total mass and COM position/velocity
                double d_m = d->m;
                node->mx  += d->mx  * d_m;
                node->my  += d->my  * d_m;
                node->mz  += d->mz  * d_m;
...
void reb_simulation_update_tree(struct reb_simulation* const r){
    ...
    if (r->central_to_zero) reb_simulation_central_to_zero(r);
}
