        ...
        case REB_GRAVITY_NONE: // Do nothing.
            for (int j=0; j<N; j++){
                ...
                if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
                    r->ri_lfada.par_jerk[j].x = 0;
                    r->ri_lfada.par_jerk[j].y = 0;
                    r->ri_lfada.par_jerk[j].z = 0;
                }
            }  
            break;
...
#pragma omp parallel for 
                for (int i=0; i<N; i++){
                    ...
                    particles[i].az = 0; 
                    if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
                        r->ri_lfada.par_jerk[i].x = 0;
                        r->ri_lfada.par_jerk[i].y = 0;
                        r->ri_lfada.par_jerk[i].z = 0;
                        //r->ri_lfada.hash_particles[i] = particles[i].hash; //debug only
                    }
                }
                        ...
                        for (int gbz=-N_ghost_z; gbz<=N_ghost_z; gbz++){
                            ...
                            for (int i=starti; i<_N_active; i++){
                                ...
                                for (int j=startj; j<i; j++){
                                    ...
                                    particles[j].az    += prefacti*dz;
                                    if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
                                        double dvx = (gb.vx+particles[i].vx) - particles[j].vx;
                                        double dvy = (gb.vy+particles[i].vy) - particles[j].vy;
                                        double dvz = (gb.vz+particles[i].vz) - particles[j].vz;
                                        double rv = dx*dvx + dy*dvy + dz*dvz;
                                        r->ri_lfada.par_jerk[j].x += prefactj*( dvx - 3.0 * rv/(_r*_r + softening2) * dx );
                                        r->ri_lfada.par_jerk[j].y += prefactj*( dvy - 3.0 * rv/(_r*_r + softening2) * dy );
                                        r->ri_lfada.par_jerk[j].z += prefactj*( dvz - 3.0 * rv/(_r*_r + softening2) * dz );
                                        r->ri_lfada.par_jerk[i].x += prefacti*( dvx - 3.0 * rv/(_r*_r + softening2) * dx );
                                        r->ri_lfada.par_jerk[i].y += prefacti*( dvy - 3.0 * rv/(_r*_r + softening2) * dy );
                                        r->ri_lfada.par_jerk[i].z += prefacti*( dvz - 3.0 * rv/(_r*_r + softening2) * dz );
                                    }
                                }
                            }
                            ...
                            for (int i=0; i<_N_real; i++){
                                for (int j=0; j<_N_active; j++){
                                    ...
                                    if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
                                        ...
                                        double dvz = (gb.vz+particles[i].vz) - particles[j].vz;
                                        double rv = dx*dvx + dy*dvy + dz*dvz;
                                        r->ri_lfada.par_jerk[i].x += prefact*( dvx - 3.0 * rv/(_r*_r + softening2) * dx );
                                        r->ri_lfada.par_jerk[i].y += prefact*( dvy - 3.0 * rv/(_r*_r + softening2) * dy );
                                        r->ri_lfada.par_jerk[i].z += prefact*( dvz - 3.0 * rv/(_r*_r + softening2) * dz );
                                    }
                                }
                            }
#endif // OPENMP
...
#pragma omp parallel for schedule(guided)
        ...
        case REB_GRAVITY_TREE:
            {
                if ((r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) && (r->ri_lfada.par_jerk == NULL)) {
                    r->ri_lfada.par_jerk = realloc(r->ri_lfada.par_jerk, sizeof(struct reb_vec3d) * N);
                    //r->ri_lfada.hash_particles = realloc(r->ri_lfada.hash_particles, sizeof(unsigned long) * N);
                }
#pragma omp parallel for schedule(guided)
                for (int i=0; i<N; i++){
                    particles[i].ax = 0;
...
#pragma omp parallel for schedule(guided)
                            for (int i=0; i<N; i++){
                                ...
                                gb.y += particles[i].y;
                                gb.z += particles[i].z;
                                gb.vx = particles[i].vx;
                                gb.vy = particles[i].vy;
                                gb.vz = particles[i].vz;
...
static void reb_calculate_acceleration_for_particle_from_cell(const struct reb_simulation* r, const int pt, const struct reb_treecell *node, const struct reb_vec6d gb) {
    ...
    // Velocidade relativa v_ij
    double dvx = gb.vx - node->mvx;
    double dvy = gb.vy - node->mvy;
    double dvz = gb.vz - node->mvz;
    // Produto escalar R_ij . v_ij
    double rv = dx*dvx + dy*dvy + dz*dvz;
    ...
    if ( node->pt < 0 ) { // Not a leaf
        if ( node->w*node->w > r->opening_angle2*r2 ){
            ...
            if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
                r->ri_lfada.par_jerk[pt].x += prefact*( dvx - 3.0 * rv/(r2 + softening2) * dx );
                r->ri_lfada.par_jerk[pt].y += prefact*( dvy - 3.0 * rv/(r2 + softening2) * dy );
                r->ri_lfada.par_jerk[pt].z += prefact*( dvz - 3.0 * rv/(r2 + softening2) * dz );
                // particles[pt].dax += prefact*( dvx - 3.0 * rv/(r2 + softening2) * dx );
                // particles[pt].day += prefact*( dvy - 3.0 * rv/(r2 + softening2) * dy );
                // particles[pt].daz += prefact*( dvz - 3.0 * rv/(r2 + softening2) * dz );
            }
