...
// int is_global_dt(double a, double b) {
//     const double eps = 1e-8;
//     double rem = fmod(a, b);
//     return fabs(rem) < eps || fabs(rem - b) < eps;
// }
...
void reb_simulation_step(struct reb_simulation* const r){
    ...
    if (r->tree_needs_update || r->gravity==REB_GRAVITY_TREE || r->collision==REB_COLLISION_TREE || r->collision==REB_COLLISION_LINETREE){
        if ((r->integrator != REB_INTEGRATOR_LEAPFROG_ADA) || (r->ri_lfada.step_counter == 0)) {
            //printf("update tree in t=%.4f | dt_global=%.4f ... \n", r->t-r->dt/2, r->ri_lfada.dt_global);
            PROFILING_START();
            reb_boundary_check(r);     
            PROFILING_STOP(PROFILING_CAT_BOUNDARY);
            //reb_message(r, 'm', "boundary check");

            // Update tree (this will remove particles which left the box)
            PROFILING_START();
            reb_simulation_update_tree(r);          
            PROFILING_STOP(PROFILING_CAT_GRAVITY);
            //reb_message(r, 'm', "tree update");
        }
    }
    if ((r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) && (r->ri_lfada.step_counter == 0)) {
        PROFILING_START();
        //reb_message(r, 'm', "plane sweep search start");
        reb_collision_search_sweep(r, r->ri_lfada.dt_global);
        PROFILING_STOP(PROFILING_CAT_COLLISION);
        //reb_message(r, 'm', "plane sweep search end");
    }


    // Search for collisions using local and essential tree. LEAPFROG ADAPTIVE
    if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
        ...
        PROFILING_STOP(PROFILING_CAT_COLLISION);
        reb_simulation_update_tree(r);
        //reb_message(r, 'm', "collision resolve");
    }
    ...
    if (r->tree_needs_update){
        // Update tree (this will remove particles which left the box)
        reb_simulation_update_tree(r);      
        //reb_message(r, 'm', "tree update: if tree_needs_update at end");    
    }
    PROFILING_STOP(PROFILING_CAT_BOUNDARY);
    ...
    // Update step counter
    if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) {
        r->ri_lfada.step_counter++;
        if (r->ri_lfada.step_counter >= r->ri_lfada.dt_factor) r->ri_lfada.step_counter = 0;
    }
...
void reb_simulation_init(struct reb_simulation* r){
    ...
    // ********** LEAPFROG ADAPTIVE
    r->ri_lfada.dt_global = 0.01;
    r->ri_lfada.dt_factor = 20;
    r->ri_lfada.step_counter = 0;
