...
struct reb_collision{
    ...
    double time;            // Leandro: usado no plane sweeping
};
...
struct reb_integrator_leapfrog_ada {
    double dt_global;
    int dt_factor;
    unsigned int mode;      // 0: hardsphere, 1: softsphere
    double Tdur;
    double t_last_acc;

    // Internal use
    int step_counter;
    unsigned int N_allocated_par_jerk;
    struct reb_vec3d* par_jerk;
    struct reb_vec3d par_jerk_reinsert;
    //uint32_t* hash_particles;
    //uint32_t hash_reinsert;
    struct reb_vec3d* par_soft_acc;
    struct reb_vec3d par_soft_reinsert;
};
...
struct reb_simulation {
    ...
    int     central_to_zero;        // 0 (default): do not set particle w/ m => 1 to 0 position, 1: set particle w/ m => 1 to 0 position
    ...
    enum {
        ...
        REB_INTEGRATOR_LEAPFROG_ADA = 5,// LEAPFROG integrator, simple, 2nd order, symplectic
    ...
    struct reb_integrator_leapfrog_ada ri_lfada;
