...
static void reb_simulation_add_local(struct reb_simulation* const r, struct reb_particle pt){
    ...
    while (r->N_allocated<=r->N){
        ...
        if (r->integrator == REB_INTEGRATOR_LEAPFROG_ADA){
            ...
            //r->ri_lfada.hash_particles = realloc(r->ri_lfada.hash_particles, sizeof(uint32_t) * r->N_allocated);
            if (r->ri_lfada.mode) r->ri_lfada.par_soft_acc = realloc(r->ri_lfada.par_soft_acc, sizeof(struct reb_vec3d) * r->N_allocated);
        }
    ...
    if ((r->integrator == REB_INTEGRATOR_LEAPFROG_ADA) && r->ri_lfada.par_jerk != NULL){
        r->ri_lfada.par_jerk[r->N] = r->ri_lfada.par_jerk_reinsert;
        if (r->ri_lfada.mode) r->ri_lfada.par_soft_acc[r->N] = r->ri_lfada.par_soft_reinsert; //talvez não precise disso aqui
        //r->ri_lfada.hash_particles[r->N] = r->ri_lfada.hash_reinsert;
        //r->ri_lfada.hash_particles[r->N] = pt.hash;
    }
